...
> 更新日期：2026-09-12（S9 加固后）  
> 本文描述**当前代码真实状态**。历史原型结论见文末附录。
...
6. UI 偏好（密度、排序等）走 `app_settings` / `set_ui_prefs`，不写 `localStorage`。
...
实现前，H 盘原型（PowerShell 备份、Python 静态清单/HTTP 服务、浏览器查看器）已完成审计。可保留为算法参考：扩展名分类、文件名日期提取、同 stem 实况配对、连拍启发式、EXIF 方向纠正、视频首帧缩略图、回收站删除、`年\月\日期\类型` 目录布局。

必须重做（当前仓库已全部重做）：硬编码 H/E 盘、Python HTTP 服务、静态清单、浏览器 `localStorage` 收藏、客户端文件列表删除、文本日志解析、仅按文件大小判重复、同步重建清单、整盘静态文件暴露。

完整原型行为描述不再维护；若需追溯，查看本文档历史版本或 `docs/requirements.md` §3。
